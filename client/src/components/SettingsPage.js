...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-100 relative overflow-hidden flex items-center justify-center p-4">
      <div className="absolute -top-20 -left-20 w-64 h-64 rounded-full bg-blue-200/40 blur-3xl"></div>
      <div className="absolute -bottom-20 -right-20 w-72 h-72 rounded-full bg-indigo-300/30 blur-3xl"></div>

      <div className="w-full max-w-6xl relative">
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white p-6">

          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <button
              onClick={() => navigate("/admin")}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white transition text-sm"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">Manage system preferences</p>
              </div>
            </div>
            ...
            <button
              onClick={loadSetting}
              disabled={loading}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white transition text-sm disabled:opacity-50"
            >
              <RefreshCw size={16} className={loading ? "animate-spin" : ""}/> {loading ? "..." : "Refresh"}
            </button>
          </div>
          ...
          <div className="rounded-2xl border shadow p-4 bg-white">
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 border p-3 flex flex-col sm:flex-row justify-between items-center gap-3">
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold">Team Feature Enabled</h3>
                  ...
                  <div className="mt-0.5 flex items-center gap-1.5 text-slate-500 text-xs">
                    <Clock3 size={14}/> Last updated: {lastUpdated ? lastUpdated : "Loading..."}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative w-14 h-8 rounded-full transition ${enabled ? "bg-green-500" : "bg-gray-300"} disabled:opacity-60`}
                >
                  <div className={`absolute top-1 h-6 w-6 rounded-full bg-white transition ${enabled ? "left-7" : "left-1"}`}></div>
                </button>
                <span className={`text-base font-bold ${enabled ? "text-green-600" : "text-red-500"}`}>
          ...
          <div className="mt-6 rounded-2xl border shadow p-4 bg-white">
            <h2 className="text-lg font-bold">Attendance Passcode</h2>
            <p className="text-slate-500 text-xs mt-0.5">Generate a passcode for users to access attendance marking.</p>

            <div className="mt-4 rounded-2xl bg-indigo-50 border p-3 flex flex-row items-center justify-between gap-3 overflow-x-auto">
              <div className="flex gap-3 items-center shrink-0">
                ...
                <div>
                  <h3 className="text-base font-bold">Today's Passcode</h3>
              ...
              <div className="flex flex-row items-center gap-2 shrink-0">
                {passcode ? (
                  <div className="h-8 px-3 flex items-center bg-white border border-indigo-200 rounded-lg text-base font-mono font-bold text-indigo-600 tracking-widest">
                    {passcode}
                  </div>
                ) : (
                  <div className="h-8 flex items-center text-xs text-slate-500">No passcode set</div>
                )}
                {passcodeExpiresAt && (
                  <div className={`h-8 flex items-center text-[10px] px-2 rounded ${timeLeft === 'Expired' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                    {timeLeft}
                  </div>
                )}
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-8 px-2 rounded-lg border border-slate-300 outline-none focus:ring-2 focus:ring-indigo-500 text-xs w-28"
                  />
                  <button
                ...
                <button
                  ...
                  disabled={passcodeLoading}
                  className="h-8 flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white px-3 rounded-lg text-xs font-semibold transition"
                >
                  <RefreshCw size={14} className={passcodeLoading ? "animate-spin" : ""} />
                  {passcodeLoading ? "Generating..." : "Generate"}
                </button>
              </div>
            </div>
          </div>

          <div className="mt-6 rounded-2xl border-l-4 border-blue-600 bg-gradient-to-r from-blue-50 to-white p-5 shadow">
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
                <Info className="text-blue-600" size={20}/>
              </div>
              <div>
                <h3 className="text-lg font-bold mb-1">About Settings</h3>
